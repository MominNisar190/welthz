 ...
 */
export async function generateMonthlyPDFReport(transactions, month, year, accountName) {
  ...
  doc.text("Account: " + accountName, 105, 28, { align: "center" });
  ...
  doc.setFontSize(10);
  doc.text("Generated: " + format(new Date(), "MMM dd, yyyy"), 105, 43, { align: "center" });

  // Prepare table data - format: +/- amount RS
  const tableData = transactions.map((transaction) => [
    format(new Date(transaction.date), "MMM dd, yyyy"),
    transaction.description || "N/A",
    transaction.category || "Uncategorized",
    transaction.type === "INCOME" ? "Income" : "Expense",
    (transaction.type === "INCOME" ? "+" : "-") + transaction.amount.toFixed(2) + " RS",
  ...
  doc.text("+" + totalIncome.toFixed(2) + " RS", 180, finalY + 18, { align: "right" });

  // Total Expense
  doc.setTextColor(redColor[0], redColor[1], redColor[2]);
  doc.text("Total Expense:", 20, finalY + 25);
  doc.text("-" + totalExpense.toFixed(2) + " RS", 180, finalY + 25, { align: "right" });

  // Net Balance
  const balanceColor = netBalance >= 0 ? greenColor : redColor;
  doc.setTextColor(balanceColor[0], balanceColor[1], balanceColor[2]);
  doc.text("Net Balance:", 20, finalY + 32);
  doc.text((netBalance >= 0 ? "+" : "-") + Math.abs(netBalance).toFixed(2) + " RS", 180, finalY + 32, { align: "right" });

  // Generate filename
  const filename = "financial-report-" + month + "-" + year + ".pdf";

  // Footer Section - Thanks message with link
  ...
  doc.setTextColor(0, 0, 0);
  doc.setFont(undefined, "normal");
  doc.text("Thanks for visiting Welthz", 105, footerY, { align: "center" });

  // Link text with highlight
  doc.setFontSize(11);
  doc.setTextColor(primaryColor[0], primaryColor[1], primaryColor[2]);
  doc.setFont(undefined, "bold");
  const linkText = "Click here for more experience";
  const linkUrl = "https://welthz.vercel.app/";

  // Add clickable link
  doc.textWithLink(linkText, 105, footerY + 7, {
    align: "center",
    url: linkUrl,
  });
  ...
  doc.line(105 - textWidth / 2, footerY + 8, 105 + textWidth / 2, footerY + 8);
